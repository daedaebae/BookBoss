const jobManager = {
    ...
    getAllJobs() {
        return Array.from(this.jobs.values()).sort((a, b) => (b as any).startTime - (a as any).startTime);
    },
    ...
    cleanup() {
        ...
        for (const [id, job] of this.jobs.entries()) {
            if (job.status !== 'running' && (job as any).endTime) {
                const diffMs = (now as any) - (job as any).endTime;
