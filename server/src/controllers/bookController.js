...
const getBooks = (req, res) => {
    ...
    if (isOwner) {
        ...
        db.query('SELECT privacy_settings FROM users WHERE id = ?', [targetUserId], (err, results) => {
            if (err || results.length === 0) return res.status(404).json({ error: 'Library not found' });

            let settings = {
            };
            try { settings = JSON.parse(results[0].privacy_settings || '{}'); } catch (e) { }

            if (!settings.share_library) { // Assuming 'share_library' is the key
...
const addBook = async (req, res) => {
    ...
    try {
        if (coverFile) {
            ...
            const filename = `cover-${Date.now()}-${crypto.randomBytes(4).toString('hex')}.jpg`;
            const uploadDir = path.join(__dirname, '../../uploads'); // Adjust path to server root uploads
            if (!fs.existsSync(uploadDir)) fs.mkdirSync(uploadDir, { recursive: true });

            const filepath = path.join(uploadDir, filename);
            await downloadImage(cover_url_remote, filepath);
...
const bulkDeleteBooks = (req, res) => {
    ...
    db.query(`DELETE FROM books WHERE id IN (${placeholders})`, ids, (err, result) => {
        ...
        res.json({ message: `${result.affectedRows} books deleted` });
    });
};
...
const bulkUpdateBooks = (req, res) => {
    ...
    db.query(`UPDATE books SET ${setClause} WHERE id IN (${placeholders})`, [...setValues, ...ids], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ message: `${result.affectedRows} books updated` });
    });
};
...
const downloadBook = (req, res) => {
    ...
    db.query('SELECT file_path, title, format FROM books WHERE id = ?', [bookId], (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        if (results.length === 0 || !results[0].file_path) return res.status(404).json({ error: 'File not found' });

        const book = results[0];
        const file = path.join(__dirname, '../../', book.file_path); // Adjust path
...
const getBookPhotos = (req, res) => {
    ...
    db.query('SELECT * FROM book_photos WHERE book_id = ? ORDER BY uploaded_at DESC', [bookId], (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(results.map(p => ({ ...p, tags: JSON.parse(p.tags || '[]') })));
    });
};
...
const addBookPhoto = (req, res) => {
    ...
    db.query(
        ...
        (err, result) => {
            if (err) return res.status(500).json({ error: err.message });
            res.status(201).json({ id: result.insertId, photoPath });
        }
    );
};
...
const deleteBookPhoto = (req, res) => {
    ...
    db.query('SELECT photo_path FROM book_photos WHERE id = ?', [photoId], (err, results) => {
        if (err || results.length === 0) return res.status(404).json({ error: 'Photo not found' });
...
const refreshMetadata = (req, res) => {
    ...
    (async () => {
        try {
            ...
            jobManager.updateJob(job.id, { total: books.length, message: `Found ${books.length} books` });

            let updatedCount = 0;
            for (let i = 0; i < books.length; i++) {
                const book = books[i];
                jobManager.updateJob(job.id, { processed: i + 1, progress: Math.round(((i + 1) / books.length) * 100) });
