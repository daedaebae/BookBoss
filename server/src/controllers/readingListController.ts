...
const getListBooks = (req, res) => {
    ...
    db.query(
        ...
        (err, lists) => {
            if (err) return res.status(500).json({ error: err.message });
            if ((lists as any[]).length === 0) return res.status(404).json({ error: 'Reading list not found' });
...
const addBookToList = (req, res) => {
    ...
    db.query(
        ...
        (err, lists) => {
            if (err) return res.status(500).json({ error: err.message });
            if ((lists as any[]).length === 0) return res.status(403).json({ error: 'Access denied' });
...
const updateReadingList = (req, res) => {
    ...
    db.query(
        ...
        (err, result) => {
            if (err) return res.status(500).json({ error: err.message });
            if ((result as any).affectedRows === 0) return res.status(404).json({ error: 'Reading list not found' });
