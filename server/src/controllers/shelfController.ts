...
const createShelf = (req, res) => {
    ...
    db.query('INSERT INTO shelves (user_id, name) VALUES (?, ?)', [userId, name], (err, result) => {
        if (err) { console.error(err); return res.status(500).json({ error: err.message }); }
        res.status(201).json({ id: (result as any).insertId, name, user_id: userId });
    });
};
...
const addBookToShelf = (req, res) => {
    ...
    db.query('SELECT id FROM shelves WHERE id = ? AND user_id = ?', [shelfId, req.user.id], (err, results) => {
        if (err || (results as any[]).length === 0) return res.status(403).json({ error: 'Shelf not found or access denied' });
...
const renameShelf = (req, res) => {
    ...
    db.query('UPDATE shelves SET name = ? WHERE id = ? AND user_id = ?', [name.trim(), shelfId, userId], (err, result) => {
        if (err) { console.error(err); return res.status(500).json({ error: err.message }); }
        if ((result as any).affectedRows === 0) return res.status(404).json({ error: 'Shelf not found' });
